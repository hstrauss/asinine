...
function get_network_info_from_url(url, _panel) {
    ...
    // var info_request = 
    Request({
        url: "http://webscale.co.za/api/get_network_info?q=" + host,
        anonymous: true,
        ...
        onComplete: function (response) {
            ...
            _panel.prt.emit('respond_info', {
                hellow: 'poop',
                dataload: response
            });
            _panel.getElementById('hostname').innerHTML = response.data['host'] + ' (' + response.data['status'] + ')';
